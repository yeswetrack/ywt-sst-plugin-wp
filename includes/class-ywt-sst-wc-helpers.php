...
class YWT_SST_WC_Helpers {
	...
	/**
	 * Calculate the total Customer Lifetime Value for a given customer.
	 *
	 * @param  int $customer_id ID van de klant.
	 * @return float
	 */
	public function calculate_total_customer_lifetime_value( $customer_id ) {
		$customer_orders = wc_get_orders( array(
			'customer_id' => $customer_id,
			'status' => 'completed', // U kunt andere statussen toevoegen indien nodig.
		));
	...
	public function get_product_brand( $product_id ) {
		$taxonomy = apply_filters( 'ywt_sst_product_brand_taxonomy', false );
		if ( false === $taxonomy ) {
			return '';
		}
	...
	public function is_enable_ecommerce() {
		return function_exists( 'WC' ) && YWT_SST_Helpers::is_enable_data_layer_ecommerce();
	}
	...
	public function is_enable_user_data() {
		return function_exists( 'WC' ) && ( WC()->customer instanceof WC_Customer ) && YWT_SST_Helpers::is_enable_data_layer_user_data();
	}
